...
1. **Lists and tables** - Most common. Separation via space, dividers, or background color (pick one). Good tables need search, filter, and sort - displaying data is only half the battle.
2. **Cards** - Charts, KPIs, toasts, notifications. Keep margins well-spaced. Use outlines in dark mode, background colors in light mode.
3. **User inputs** - Forms, settings, modals with form fields. Can appear inside cards or tables.
4. **Tabs** - Add views without cluttering the sidebar. Different perspectives on related data without leaving context.
