...
3. On **Products**, enable **Sign In with LinkedIn using OpenID Connect**.
4. Apply for **Community Management API** access.
5. Development tier is the initial approved tier. It is limited to 500 app calls and 100 calls per
...
6. Apply separately for **Standard tier** before production use. LinkedIn approves partners at its
   discretion and does not guarantee an upgrade.
7. Copy the app's Client ID and Client Secret into the Connection, then authorize a member.

The package requests the current `openid` and `profile` scopes to identify the member, plus
`r_organization_social`, `w_organization_social`, and `w_member_social` for Community Management.
LinkedIn grants the Community Management scopes only to approved apps.
...
| Tool | Access |
| --- | --- |
| `linkedin_current_member` | Read the authorized member's OpenID Connect profile |
...
The member identity check uses LinkedIn's OpenID Connect userinfo endpoint and its `sub` subject.
That subject identifies the member for this client. Organization IDs remain separate operation
targets and are not inferred from the member subject.
Community Management requests pin `Linkedin-Version: 202608` and
`X-Restli-Protocol-Version: 2.0.0`. LinkedIn returns a new post's URN only in the `x-restli-id`
response header. The current OIM JSON result contract does not expose response headers, so the
create Tool returns an empty object even when creation succeeds.
