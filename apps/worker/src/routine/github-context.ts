...
export class InstallationScopeGitHubContextResolver implements GitHubContextResolver {
  ...
  async resolve(intent: ToolIntent): Promise<GitHubEffectContext | undefined> {
    ...
    if (matches.length > 1) {
      // Two active installations both list the same repository — e.g. a stale row from a
      // reinstall, or a transient org-transfer overlap. Guessing which one to dispatch through
      // would silently pick a token/permission scope the caller never chose; refuse instead, same
      // as the no-match case below.
      this.log?.warn(
