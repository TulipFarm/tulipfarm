...
describe("usePageSearch", () => {
  ...
  it("searches scoped to the space when scope is 'space'", async () => {
    searchPages.mockResolvedValue([hit("d2", "Orders")]);
    const { result } = renderHook(() => usePageSearch("b1"));
    expect(result.current.scope).toBe("space"); // defaults to space when a space is active

    act(() => result.current.setQuery("orders"));
    await waitFor(() =>
      expect(searchPages).toHaveBeenCalledWith("orders", { spaceId: "b1", limit: 10 })
    );
    await waitFor(() => expect(result.current.results[0].title).toBe("Orders"));
  });

  it("drops the space filter when scope is 'all'", async () => {
    const { result } = renderHook(() => usePageSearch("b1"));
    act(() => {
    ...
    await waitFor(() =>
      expect(searchPages).toHaveBeenCalledWith("q", { spaceId: undefined, limit: 10 })
    );
  });

  it("dedupes results by pageId", async () => {
    searchPages.mockResolvedValue([hit("dup", "A"), hit("dup", "A again"), hit("d3", "B")]);
    const { result } = renderHook(() => usePageSearch(null));
    act(() => result.current.setQuery("x"));
    await waitFor(() => expect(result.current.results).toHaveLength(2));
    expect(result.current.results.map((r) => r.pageId)).toEqual(["dup", "d3"]);
  });

  it("defaults scope reactively to the active space; an explicit choice sticks across navigation", () => {
    const { result, rerender } = renderHook(({ b }) => usePageSearch(b), {
      initialProps: { b: null as string | null },
    });
    expect(result.current.scope).toBe("all"); // no space → all

    rerender({ b: "b1" }); // navigate into a space (palette is not remounted)
    expect(result.current.scope).toBe("space"); // reactive default follows the space
