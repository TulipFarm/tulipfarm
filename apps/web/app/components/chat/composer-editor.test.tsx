...
const linkConfig = vi.hoisted<{
  extend?: { inclusive: () => boolean };
  configure?: { autolink?: boolean };
}>(() => ({}));
vi.mock("@tiptap/extension-link", () => ({
  default: {
    extend: (config: { inclusive: () => boolean }) => {
      linkConfig.extend = config;
      return {
        configure: (options: { autolink?: boolean }) => {
          linkConfig.configure = options;
          return {};
        },
      };
    },
...
test("the effort preset can default from the active agent", async () => {
  const user = userEvent.setup();
  const onSend = vi.fn();
  render(<ComposerEditor onSend={onSend} defaultModel="thorough" />);
...
test("choosing a file shows a removable chip and sends its id once uploaded", async () => {
  ...
  const { container } = render(<ComposerEditor onSend={onSend} />);
