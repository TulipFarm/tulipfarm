...
| `app/components/integrations/integration-operations.tsx`, `app/lib/integration-operations.ts` | OIM provider detail mounts `IntegrationOperations({ integrationKey })` for authorized operational evidence and selected-scope Knowledge subscription controls; legacy providers never request this endpoint. |
