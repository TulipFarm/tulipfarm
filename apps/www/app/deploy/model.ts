import { DOCS_URL } from "@tulipfarm/constants/site";
 ...
 */
export function resolveSiteUrl<T>(value: T, siteUrl: string, docsUrl = DOCS_URL): T {
  if (typeof value === "string") {
    return value.replaceAll("{{SITE_URL}}", siteUrl).replaceAll("{{DOCS_URL}}", docsUrl) as T;
  }
  if (Array.isArray(value))
    return value.map((entry) => resolveSiteUrl(entry, siteUrl, docsUrl)) as T;
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value).map(([key, entry]) => [key, resolveSiteUrl(entry, siteUrl, docsUrl)])
    ) as T;
  }
  return value;
}
