...
| `JWT_SECRET` | — | Required, validated as 32-byte base64 at boot. Reserved for request signing — no feature is wired to it yet (API tokens are stored as hashes, not JWTs — see [Hashing](/docs/security/hashing)). Generate with `openssl rand -base64 32`. |
