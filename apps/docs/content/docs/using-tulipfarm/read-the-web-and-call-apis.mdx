...
Unlike a page read, an API response is never summarised. The whole thing comes back, headers
included, exactly as the other site sent it. A response is already compact, and the field names an
agent needs in order to work with it are the first thing a summary would drop. A request is never
remembered either, so a second question about the same API is a second real request, which is why
an agent is told to read the response it already has rather than ask again.
...
Only a page read is ever summarised. Most pages come back small enough to read whole. A large one
is summarised once before the agent reads it, using the cheapest model configured on your instance,
against the note the agent sent with the read, or against your own last message, if it sent none.
...
An oversized API response is cut short rather than summarised, and the agent is told it was cut.
The fix is a narrower request (fewer fields, or one page of results at a time), not a second
...
Every call appears in the [run](/using-tulipfarm/runs). The URL, the method, whether a
credential was used, and what came back. Writes appear as
[approvals](/using-tulipfarm/approvals) before they happen.

If something looks wrong, see [when things go wrong](/using-tulipfarm/when-things-go-wrong).
