...
- A risky skill is still risky, because the agent running it may be permitted. That is why every
  ...
  a human one. See [Install a skill](/using-tulipfarm/install-a-skill).
...
[bounding which skills an agent may use](/using-tulipfarm/agents#bounding-which-skills-an-agent-may-use).
...
[Routines](/using-tulipfarm/routines).
...
See [Knowledge](/using-tulipfarm/knowledge) and [Memory](/using-tulipfarm/memory).
...
You are not locked in. Both are written into [the soul](/using-tulipfarm/what-a-soul-is), so both can
