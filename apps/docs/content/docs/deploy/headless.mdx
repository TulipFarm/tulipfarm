...
Headless seeding stores the LLM key and writes a default `llm` block in `soul.yaml` with the provider in
all three tiers pointing to a default model, from which the `fast`, `balanced`, and `thorough`
effort presets are derived. To adjust the model or add a fallback chain, sign in as the admin and
