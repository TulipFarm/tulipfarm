...
assembler and the *real* Tool loop, scores deterministic **Assertions**, and prints a **Scorecard**.

Manually triggered before a release. Never runs in the product; nothing here is reachable at runtime.

## Read on / Skip

**Read on** if your task touches: eval Cases, Assertion kinds, the Corpus hash, Sweep execution,
...
| Path | Owns |
| --- | --- |
| `src/case.ts` | `EvalCase`, the `Assertion` union, loop limits. The vocabulary everything else reads. |
...
- **The assembler must stay in the path.** `AgentLoopInput.messages` is *already assembled*, so a
  ...
  `runTrial` calls `assembleSystemPrompt` itself; `prompt_contains` is the assertion that proves it.
- **Assertions are data, never functions.** That is what lets the Corpus be content-hashed and a
...
- **A Case that asserts nothing is rejected at load,** and an `unasserted` Trial fails the CLI exit
  code. `[].every(...)` is `true`, so an empty `expect` would otherwise clear the release gate
  having checked nothing.
- **A malformed Assertion is rejected at load,** per-kind, by `ASSERTION_FIELDS`. A missing
