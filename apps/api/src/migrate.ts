...
export async function runDataMigrations(db: Db): Promise<void> {
  ...
  const pending = DATA_MIGRATIONS.filter((m) => m.version > currentVersion).sort((a, b) => a.version - b.version)
  ...
  for (const migration of pending) {
    ...
    try {
      ...
      await meta.updateOne({ _id: VERSION_DOC_ID }, { $set: { version: migration.version } }, { upsert: true })
      console.log(`✅ Data migration v${migration.version} applied`)
    } catch (error) {
      console.error(`❌ Data migration v${migration.version} failed: ${error instanceof Error ? error.message : String(error)}`)
      process.exit(1)
    }
  }
}
