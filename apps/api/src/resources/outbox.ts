...
import { DOMAIN_EVENTS, type ResourceEventPayload, withTransaction } from "@tulipfarm/storage";
...
export const RESOURCE_SIDE_EFFECT_STORAGE_STATEMENTS = [
  `CREATE TABLE IF NOT EXISTS resource_create_requests (
    resource_type   text NOT NULL,
    caller_id       text NOT NULL,
    idempotency_key text NOT NULL,
    resource_id     uuid NOT NULL,
    created_at      timestamptz NOT NULL DEFAULT now(),
    PRIMARY KEY (resource_type, caller_id, idempotency_key)
  )`,
  `CREATE TABLE IF NOT EXISTS resource_side_effect_outbox (
    id          uuid PRIMARY KEY,
    effect      jsonb NOT NULL,
    status      text NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'leased', 'delivered', 'quarantined')),
  ...
  `CREATE INDEX IF NOT EXISTS resource_side_effect_outbox_claim_idx
     ON resource_side_effect_outbox (status, lease_until, created_at)`,
] as const;
...
export async function writeResourceSideEffect(
  ...
  await transaction.query(
    `INSERT INTO resource_side_effect_outbox (id, effect) VALUES ($1, $2::jsonb)`,
    [id, JSON.stringify(effect)]
  );
}
...
export class ResourceSideEffectOutbox {
  ...
  async claim(
    ...
    return withTransaction(this.database, async (transaction) => {
      const result = await transaction.query<OutboxRow>(
        `WITH candidates AS (
           SELECT id
             FROM resource_side_effect_outbox
            WHERE status = 'pending' OR (status = 'leased' AND lease_until <= now())
            ORDER BY created_at, id
            FOR UPDATE SKIP LOCKED
            LIMIT $1
         )
         UPDATE resource_side_effect_outbox AS outbox
            SET status = 'leased', attempts = outbox.attempts + 1, lease_owner = $2,
                lease_until = now() + ($3 * interval '1 millisecond')
           FROM candidates
          WHERE outbox.id = candidates.id
         RETURNING outbox.id, outbox.effect, outbox.attempts`,
        [limit, owner, leaseMs]
      );
      return result.rows.map((row) => ({ id: row.id, effect: row.effect, attempts: row.attempts }));
    });
  }

  async complete(id: string, owner: string): Promise<boolean> {
    const result = await this.database.query(
      `UPDATE resource_side_effect_outbox
          SET status = 'delivered', lease_owner = NULL, lease_until = NULL, delivered_at = now()
  ...
  async fail(id: string, owner: string, maxAttempts: number): Promise<boolean> {
    const result = await this.database.query(
      `UPDATE resource_side_effect_outbox
          SET status = CASE WHEN attempts >= $3 THEN 'quarantined' ELSE 'pending' END,
              lease_owner = NULL,
              lease_until = NULL
        WHERE id = $1 AND status = 'leased' AND lease_owner = $2
        RETURNING id`,
...
function eventName(kind: ResourceMutationKind): string {
  if (kind === "create") return DOMAIN_EVENTS.RESOURCE_CREATED;
  if (kind === "update") return DOMAIN_EVENTS.RESOURCE_UPDATED;
  return DOMAIN_EVENTS.RESOURCE_DELETED;
}
...
export async function deliverResourceSideEffect(
  ...
  events?.emit(eventName(effect.kind), payload);
}
