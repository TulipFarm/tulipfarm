...
describe("auditPersistedSchedules", () => {
  it("logs nothing when every persisted schedule is still actively registered", async () => {
    const boss = fakeBoss([{ name: "maintenance-sweep" }, { name: "obs-event-prune" }]);
    const log = fakeLog();

    await auditPersistedSchedules(boss, ["maintenance-sweep", "obs-event-prune"], log);
  ...
  it("logs loudly for a schedule no longer among the active queue names", async () => {
    ...
    const boss = fakeBoss([{ name: "maintenance-sweep" }, { name: "slack-knowledge-sync" }]);
    const log = fakeLog();

    await auditPersistedSchedules(boss, ["maintenance-sweep"], log);
