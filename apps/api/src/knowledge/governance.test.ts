...
describe("buildGovernanceBlock", () => {
  ...
  it("wraps docs in a governance-knowledge block with titles", () => {
    const block = buildGovernanceBlock([gdoc({ title: "Code of Conduct", plainText: "be nice" })]);
  ...
  it("skips a doc whose body exceeds the per-doc cap (not truncated)", () => {
    const big = "x".repeat(PER_DOC_CHAR_CAP + 1);
    const block = buildGovernanceBlock([
      gdoc({ title: "Big", plainText: big }),
      gdoc({ title: "Small", plainText: "ok" }),
    ]);
    expect(block).not.toContain("Big");
    expect(block).toContain("Small");
  });

  it("stops adding docs once the block cap would be exceeded", () => {
    const body = "y".repeat(PER_DOC_CHAR_CAP);
    const docs = Array.from({ length: 10 }, (_, i) => gdoc({ title: `D${i}`, plainText: body }));
    const block = buildGovernanceBlock(docs);
  ...
  it("orders domain-scoped before tenant-wide and excludes other domains", () => {
    const block = buildGovernanceBlock(
      [
        gdoc({ title: "Tenant", plainText: "t", domain: null }),
        gdoc({ title: "Ops", plainText: "o", domain: "ops" }),
        gdoc({ title: "HR", plainText: "h", domain: "hr" }),
      ],
      "ops"
    );
