...
import type { PendingMemory, ResolvePendingResult } from "./confirm";
import { resolvePendingMemory } from "./confirm";
...
export class GuardrailsCandidateScreen implements MemoryCandidateScreen {
  constructor(
    private readonly guardrails: InputGuardrailPort,
...
/**
 * The slice of the guardrails service extraction needs. Narrowed to keep this package off
 * `@tulipfarm/agent-runtime`, which the durable runtime composes separately.
 */
export interface InputGuardrailPort {
  runInput(
    text: string,
