...
import { assertRunActive, RunInterruptedError } from "../interruption";
import { RUN_EXECUTOR_PRINCIPAL_REF } from "../invocation/gateway";
...
import { routineBudgetScopedLimits } from "./limit-enforcement";
import { InMemoryStateRetryStore, retryBackoffMs, type StateRetryStore } from "./retry";
import { type RoutineStateScheduler, routineOccurrenceKey } from "./scheduling";
import { planActionDispatch } from "./states/action";
import { agentOutputSchema, planAgentInvocation } from "./states/agent";
import { decideBranch } from "./states/branch";
import { computeStateOutput } from "./states/compute";
import { planEmit } from "./states/emit";
import { planScriptExecution } from "./states/script";
import { type StepOutcome, stateOutcome } from "./states/step";
import { planToolDispatch } from "./states/tool";
import { resolveErrorPath } from "./states/wait-plan";
import { applyRoutineToolStateOutcome, type RoutineToolExecutionOutcome } from "./tool-outcome";
import {
  type ChildRoutinePort,
  openApproval,
  openChildRoutine,
  openWait,
  type RoutineApprovalPort,
...
export type RoutineRunExecutor = (
  run: PersistedRun,
  signal?: AbortSignal
) => Promise<{
  readonly status: "succeeded" | "failed" | "waiting" | "needs_reconciliation" | "cancelled";
  readonly errorEvidenceRef?: string;
}>;

export interface RoutineStateTransitionPort {
  transition(input: {
    businessId: string;
    runId: string;
    stateKey: string;
...
export interface RoutineExecutionDefinition<TBundle> {
  readonly document: Parameters<typeof compileRoutine>[0];
  readonly bundle: TBundle;
}

export interface RoutineEmitPort {
  emit(input: {
    businessId: string;
    runId: string;
    stateKey: string;
...
export interface RoutineScriptExecutionPort {
  execute(input: {
    runId: string;
    stateKey: string;
    plan: ReturnType<typeof planScriptExecution>;
  }): Promise<
...
export interface RoutineActionExecutionPort {
  execute(input: {
    businessId: string;
    runId: string;
    stateKey: string;
...
export interface RoutineAgentExecutionPort<TBundle> {
  execute(input: {
    businessId: string;
    runId: string;
    stateKey: string;
    ...
    attempt: number;
    signal?: AbortSignal;
    plan: ReturnType<typeof planAgentInvocation>;
    outputSchema?: Exclude<ReturnType<typeof agentOutputSchema>, undefined>;
    scopedLimits?: readonly Exclude<ReturnType<typeof routineBudgetScopedLimits>, undefined>[];
    bundle: TBundle;
  }): Promise<
    | { readonly kind: "succeeded"; readonly output: unknown }
...
export interface RoutineToolExecutionRequest<TBundle, TAuthorityLayer> {
  ...
  readonly plan: ReturnType<typeof planToolDispatch>;
  readonly requesterPrincipalId: string;
  readonly bundle: TBundle;
  readonly authorityLayers: readonly TAuthorityLayer[];
}

export interface RoutineToolExecutionPort<TBundle, TAuthorityLayer> {
  execute(
    request: RoutineToolExecutionRequest<TBundle, TAuthorityLayer>
  ): Promise<RoutineToolExecutionOutcome>;
  replaySettled(
    request: RoutineToolExecutionRequest<TBundle, TAuthorityLayer>
  ): Promise<RoutineToolExecutionOutcome>;
}
...
export interface RoutineExecutorOptions<TBundle, TAuthorityLayer> {
  readonly definitions: {
    load(run: PersistedRun): Promise<RoutineExecutionDefinition<TBundle>>;
  };
  ...
  readonly transitions: RoutineStateTransitionPort;
  readonly waits: RoutineWaitPort;
  /** Absent means `tool` States park; there is no second external-effect path. */
  readonly tools?: RoutineToolExecutionPort<TBundle, TAuthorityLayer>;
  ...
  readonly scripts?: RoutineScriptExecutionPort;
  /** Runs an `action` State's runtime Tool; absent refuses the State by name. */
  readonly actions?: RoutineActionExecutionPort;
  /** Absent means `agent` States park; the executor will not invent answers. */
  readonly agents?: RoutineAgentExecutionPort<TBundle>;
  ...
  readonly emissions?: RoutineEmitPort;
  /** Fail-closed authority; this process may not mint authority of its own. */
  readonly authority?: (run: PersistedRun, state: CompiledState) => readonly TAuthorityLayer[];
  /**
...
export function createRoutineExecutor<TBundle, TAuthorityLayer>(
  options: RoutineExecutorOptions<TBundle, TAuthorityLayer>
): RoutineRunExecutor {
...
interface ExecutionContext<TBundle, TAuthorityLayer> {
  ...
  readonly options: RoutineExecutorOptions<TBundle, TAuthorityLayer>;
  readonly retries: StateRetryStore;
  readonly concurrency: StateConcurrencyStore;
  readonly contention: StateContentionStore;
  readonly delay: (ms: number) => Promise<void>;
...
class RoutineExecution<TBundle, TAuthorityLayer> {
  ...
  constructor(private readonly ctx: ExecutionContext<TBundle, TAuthorityLayer>) {}
  ...
  private outputFor(
    ...
    if (state.type === "tool") return null;
    ...
    for (let step = 0; step <= this.ctx.routine.order.length; step += 1) {
      ...
      try {
        if (row.status === "succeeded") {
          ...
          if (state.type === "tool") {
            const replayed = await this.replayTool(state, key, scope);
            if (replayed.kind !== "outcome") return replayed.kind;
  ...
  private async runTool(
    ...
    if (port === undefined) throw new RoutineExecutionRefusal("unsupported_state", state.name);
    const result = await this.withRetry(state, key, () =>
      port.execute(this.toolRequest(state, key, scope))
    );
    this.assertActive();
    return this.applyToolOutcome(state, key, result, false);
  }

  private async replayTool(
    state: CompiledState,
    key: string,
    scope: Readonly<Record<string, unknown>>
  ): Promise<{ kind: "outcome"; outcome: StepOutcome } | { kind: ChainOutcome }> {
    ...
    const result = await port.replaySettled(this.toolRequest(state, key, scope));
    this.assertActive();
    const outcome = await this.applyToolOutcome(state, key, result, true);
    return outcome === null
  ...
  private toolRequest(
    ...
    key: string,
    scope: Readonly<Record<string, unknown>>
  ): RoutineToolExecutionRequest<TBundle, TAuthorityLayer> {
    const leaseOwner = this.ctx.run.leaseOwner;
    if (leaseOwner === null) throw new RunInterruptedError();
    return {
      businessId: this.ctx.run.businessId,
      runId: this.ctx.run.id,
      stateKey: key,
  ...
  private async applyToolOutcome(
    ...
    settledReplay: boolean
  ): Promise<StepOutcome | ChainOutcome | null> {
    return applyRoutineToolStateOutcome<ChainOutcome>(
      state,
      result,
      {
        complete: (output) => {
          this.produced.set(key, output);
        },
        ...
        wait: async (reason) => {
          await this.transition(key, "running", "waiting", `routine:${reason}`);
          return "waiting";
        },
        reconcile: async (evidenceRef) => {
          if (!settledReplay) await this.park(key, evidenceRef);
          return "needs_reconciliation";
        },
        fail: (code) => this.refuse(key, code),
      },
      { settledReplay }
    );
  }
