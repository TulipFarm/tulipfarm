...
- `SecretBroker` is default-deny, clamps TTL/uses, preserves `get()` cache semantics for legacy
  leases, resolves revision-pinned Connection values uncached, emits metadata only, and revokes by
  Secret, lease, or all.
- A network lease scope binds the caller, Connection, credential slot, destination, Run, and
  active Skill as well as the exact Secret reference; scope equality is exact across every field.
