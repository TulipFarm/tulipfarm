...
| `src/secret-reference.ts` | Opaque Secret reference validation and storage-key mapping. |
| `src/mcp-account-secrets.ts` | Revision-pinned account Secrets; the whole MCP session stays inside one bounded lease callback. |
...
- `SecretBroker` is default-deny, clamps TTL/uses, preserves the provider's cache semantics,
  emits metadata only, and revokes by Secret, lease, or all.
- `McpAccountSecrets` pins durable revisions, resolves values uncached and rechecks live authority
  around redemption; generic leases must not replace that account gate.
- Lease scope equality is exact across the Secret reference, Tool, Integration, target, Run,
  State, purpose, principal, destination and active Skill.
