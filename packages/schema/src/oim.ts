...
function prohibitedFileIssue(path: string): string | undefined {
  ...
  if (path.toLowerCase() === "manifest.yml") {
...
export function oimManifestIssues(manifest: OimManifest): string[] {
  ...
  for (const operation of manifest.operations) {
    ...
    if (operation.source.type === "http" && oimOriginPlaceholder(operation.source.baseUrl)) {
      // A templated host has no literal hostname to judge; auth.allowedOriginHosts carries that
      // guarantee instead, and is checked below.
    } else if (operation.source.type === "http") {
...
/** Parse and fully validate an OIM `manifest.yml`. */
...
/**
 * Validate an exact companion-file set. The `manifest.yml` entry point is supplied separately and
