...
describe("OIM package integrity", () => {
  ...
  it("reserves oim.yml as the only package entry point", () => {
    const manifest = valid();
    manifest.files = [
      {
        path: "oim.yml",
    ...
    expect(oimManifestIssues(manifest)).toContain(
      "files: oim.yml is the OIM entry point and cannot be a companion file"
    );

    manifest.files[0].path = "manifest.yml";
    expect(oimManifestIssues(manifest)).toContain(
      "files: manifest.yml is not allowed in an OIM package"
    );
  });
...
describe("OIM semantic compatibility", () => {
  it("rejects a required native HTTP parameter added in a minor release", () => {
    const previous = valid();
    previous.metadata.version = "1.0.0";
    if (previous.operations[0].source.type !== "http") throw new Error("fixture");
    previous.operations[0].source.path = "/items";

    const next = structuredClone(previous);
    next.metadata.version = "1.1.0";
    next.profiles.core = "1.1";
    if (next.operations[0].source.type !== "http") throw new Error("fixture");
    ...
    expect(oimCompatibilityIssues(previous, next)).toContain(
      "operations: current-weather HTTP parameter query tenant added required input"
    );

    next.operations[0].source.parameters[0].required = false;
    expect(oimCompatibilityIssues(previous, next)).toEqual([]);
  });

  it("rejects removing, renaming, or binding an Agent-provided HTTP parameter", () => {
    const previous = valid();
    if (previous.operations[0].source.type !== "http") throw new Error("fixture");
    ...
    for (const parameters of [
      [],
      [{ name: "account", in: "query" as const, schema: { type: "string" } }],
      [
        {
          name: "tenant",
          in: "query" as const,
      ...
      if (next.operations[0].source.type !== "http") throw new Error("fixture");
      next.operations[0].source.parameters = parameters;

      expect(oimCompatibilityIssues(previous, next)).toContain(
        "operations: current-weather HTTP parameter query tenant was removed or became bound"
      );
    }
  });

  it("rejects stricter HTTP parameter input and accepts widening", () => {
    const previous = valid();
    if (previous.operations[0].source.type !== "http") throw new Error("fixture");
    previous.operations[0].source.parameters = [
      {
    ...
    if (required.operations[0].source.type !== "http") throw new Error("fixture");
    required.operations[0].source.parameters?.splice(0, 1, {
      name: "units",
      in: "query",
      required: true,
      schema: { type: "string", enum: ["metric", "imperial"] },
    ...
    expect(oimCompatibilityIssues(previous, required)).toContain(
      "operations: current-weather HTTP parameter query units became required"
    );

    const narrowed = structuredClone(previous);
    narrowed.metadata.version = "1.3.0";
    if (narrowed.operations[0].source.type !== "http") throw new Error("fixture");
    narrowed.operations[0].source.parameters?.splice(0, 1, {
      name: "units",
      in: "query",
      schema: { type: "string", enum: ["metric"] },
    });
    expect(oimCompatibilityIssues(previous, narrowed)).toContain(
      "operations: current-weather HTTP parameter query units narrowed enum value imperial"
    );

    const widenedPrevious = structuredClone(previous);
    if (widenedPrevious.operations[0].source.type !== "http") throw new Error("fixture");
    widenedPrevious.operations[0].source.parameters?.splice(0, 1, {
      name: "units",
      in: "query",
      required: true,
      schema: { type: "string", enum: ["metric"] },
    });
    const widened = structuredClone(widenedPrevious);
    widened.metadata.version = "1.3.0";
    if (widened.operations[0].source.type !== "http") throw new Error("fixture");
    widened.operations[0].source.parameters?.splice(0, 1, {
      name: "units",
      in: "query",
      required: false,
      schema: { type: "string", enum: ["metric", "imperial"] },
    });
    expect(oimCompatibilityIssues(widenedPrevious, widened)).toEqual([]);
  });

  it("treats path parameters as required and tracks parameter locations", () => {
    const previous = valid();
    if (previous.operations[0].source.type !== "http") throw new Error("fixture");
    previous.operations[0].source.path = "/items";
    previous.operations[0].source.parameters = [
    ...
    moved.metadata.version = "1.3.0";
    if (moved.operations[0].source.type !== "http") throw new Error("fixture");
    moved.operations[0].source.path = "/items/{tenant}";
    moved.operations[0].source.parameters?.splice(0, 1, {
      name: "tenant",
      in: "path",
      required: true,
      schema: { type: "string" },
    });
    expect(oimCompatibilityIssues(previous, moved)).toEqual(
      expect.arrayContaining([
        "operations: current-weather HTTP parameter query tenant was removed or became bound",
        "operations: current-weather HTTP parameter path tenant added required input",
      ])
    );
  });
...
describe("OIM Events profile", () => {
  ...
  it.each(["hmac_sha256", "hmac_sha512", "ed25519", "rsa_sha256"] as const)(
    "accepts a timestamp authenticated by %s",
    (scheme) => {
      const manifest = withEvents();
      const events = {
        ...manifest.events,
  ...
  it.each([
    ...
    ["hmac_sha512", "{body}"],
    ["ed25519", "prefix:{body}"],
    ["rsa_sha256", "{body}:suffix"],
  ] as const)("rejects an unauthenticated freshness timestamp for %s", (scheme, signingInput) => {
    const manifest = withEvents();
    const events = {
      ...manifest.events,
      verification: {
        ...
        signatureHeader: "x-signature",
        signatureEncoding: "hex",
        ...(signingInput === undefined ? {} : { signingInput }),
        timestampHeader: "x-timestamp",
      },
    };

    expect(oimManifestIssues({ ...manifest, events } as OimManifest)).toContain(
      "events: timestampHeader requires signingInput to include {timestamp}"
    );
  });
  ...
  it.each([
  ...
  ] as const)("rejects timestamp freshness that $scheme cannot authenticate", (verification) => {
    const manifest = withEvents();
    const events = {
      ...manifest.events,
      verification: {
    ...
    expect(oimManifestIssues({ ...manifest, events } as OimManifest)).toContain(
      `events: ${verification.scheme} cannot authenticate timestampHeader`
    );
  });
