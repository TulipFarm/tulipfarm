...
export class AgentLoop {
  ...
  async run(input: AgentLoopInput): Promise<AgentLoopOutcome> {
     ...
     * recovered from a durable park: their proposal is already in the recovered transcript,
    ...
    const dispatchCalls = async (
      ...
      > => {
        ...
        if (dispatched.status === "needs_reconciliation") {
      ...
      while (index < calls.length) {
        ...
        if (tool.mutating !== false) {
          ...
          if (outcome.kind === "reconcile") {
            return finish({ status: "needs_reconciliation", ...counters }, "failed");
          }
      ...
      if (park !== undefined) {
        // Why this Turn stopped, in words the model can act on. All parks replay the parked call
        ...
        const parked =
          park.kind === "approval"
            ...
            : park.kind === "retry"
              ? {
                  waitingFor: "a provider retry timer opened by an earlier call",
                  alsoPending:
                    "If it also hit a provider limit, its retry timer is already pending — do not re-issue it.",
                }
              : {
        ...
        // strictly before executing the Tool. Child and retry parks already performed durable
        // work, so their replay must be idempotent on `(runId, callId)`.
