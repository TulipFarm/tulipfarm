...
export interface OimFixtureContinuation {
...
export interface OimFixtureRunOptions {
  /** Continuation state keyed by fixture Case name; the Case uses the canonical input token. */
  readonly continuations?: Readonly<Record<string, OimFixtureContinuation>>;
  readonly now?: () => number;
}
...
class RecordingFixtureHttp implements EgressHttpPort {
  ...
  async send(request: EgressHttpRequest): Promise<IntegrationHttpResponse> {
    ...
    if (
      ...
      if (request.maxResponseBytes !== undefined && bytes.byteLength > request.maxResponseBytes) {
...
async function runFixture(
  ...
  const fixturePaginationRuntime = (toolId: string, pagination: OimPagination | undefined) => {
    if (pagination === undefined) return undefined;
    const continuation = options.continuations?.[fixture.name];
    const seed: OimFixtureContinuationSeed | undefined =
    ...
    return createOimFixturePaginationRuntime(options.now ?? (() => 0), seed);
  };
  ...
  try {
    ...
    if (operation.source.type === "http") {
      ...
      const paginationRuntime = fixturePaginationRuntime(tool.toolId, tool.pagination);
      compiled = {
  ...
  for (const file of manifest.files ?? []) {
    ...
    try {
      ...
      for (const fixture of suite.cases) {
        try {
          await runFixture(manifest, companions, fixture, options);
