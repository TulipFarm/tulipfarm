...
describe("redactCredentialFields", () => {
  ...
  it("does not expose values below the response depth bound", () => {
    let nested: Record<string, unknown> = { access_token: "must-not-leak" };
    for (let depth = 0; depth < 40; depth += 1) nested = { child: nested };

    const redacted = redactCredentialFields(nested);

    expect(JSON.stringify(redacted)).not.toContain("must-not-leak");
  });
