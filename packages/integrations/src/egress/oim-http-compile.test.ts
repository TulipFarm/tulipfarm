...
describe("compileOimHttpOperations pagination", () => {
  ...
  it("wraps a paginated array schema in the runtime items envelope", () => {
    const input = paginated({ type: "page", requestParameter: "page" });
    const operation = input.operations[0];
    if (operation === undefined) throw new Error("fixture missing operation");
    const arraySchema = {
      type: "array",
    ...
    operation.response = { maxBytes: operation.response.maxBytes, schema: arraySchema };

    expect(compileOimHttpOperations(input)[0]?.contract.spec.outputSchema).toEqual({
      type: "object",
      properties: {
        items: arraySchema,
