...
function destinationOf(
  ...
  openApiDocuments: ReadonlyMap<string, unknown>
): string | undefined {
  const { source } = operation;
  if (source.type === "openapi" && source.baseUrl === undefined) {
    if (!openApiDocuments.has(source.file)) return undefined;
    ...
    return compiled === undefined ? undefined : new URL(compiled.binding.baseUrl).host;
  }
  const raw =
    source.type === "graphql"
      ? source.url
      : "baseUrl" in source && typeof source.baseUrl === "string"
        ? source.baseUrl
        : undefined;
