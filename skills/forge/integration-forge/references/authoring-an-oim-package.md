...
profiles:
  ...
  auth: "1.1" # required for a provider-verified Connection
...
operations:
  - id: acme-whoami
    name: acme_whoami
    description: Identify the Acme user authenticated by this Connection.
    effect: read
...
- **Profiles follow blocks.** An `auth:` block without `profiles.auth` is refused, and so is every
  other block without its profile. Use Auth 1.1 and declare `auth.verification`; Auth 1.0 packages
  can be reviewed but are not offered for new Connections.
- **Verification is a fixed read.** Name a native HTTP `GET` operation with no required Agent
  input. Its declared Credential slots must exactly match the operation. Require a real provider
  subject for `identified` evidence, or use `validity_only` without inventing an identity.
- **Identity comes from the provider.** A subject or tenant may come only from a successful
  verification response, a package-owned HTTPS issuer, or required configuration checked against
  that response. Never use a display name, optional domain hint, bot owner, or browser callback
  value as identity.
...
provider that names the event only in a header cannot be typed by an authored package. Do not add a
normalization hook to work around that limit; authored packages are declarative only.

## Shipped examples

Twenty-four complete packages live in the repository and are validated on every run.
