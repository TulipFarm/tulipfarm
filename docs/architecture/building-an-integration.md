...
A read/write integration is a manifest plus an OpenAPI document — no TypeScript — that compiles to a
handful of working agent Tools. A **channel** is the same, plus a sandboxed classifier, yielding a
bot that holds conversations. The bundled [`integrations/`](../../integrations/) directory holds
working manifests you can read alongside this guide.
...
egress:
  ...
  auth:
    token_env: ACME_ACCESS_TOKEN     # a connection env var, sealed at connect time
  headers:
    Acme-Version: "2024-01-01"
  operations:
    - operation: search       # operationId in the spec
      name: search            # agents call this as <slug>_search
      description: Search Acme for pages and records by title. Use this first when you
...
*for* and what it needs first — "Search Acme for pages by title. Use this first when you know a
page by name but not by id; nearly every other Acme tool needs an id."
...
A few providers put the credential in the URL instead of a header — some bot APIs are shaped like
...
encoding a `123:ABC` token into `123%3AABC` would produce a URL the provider 404s — so a
...
Some providers put an install-specific id in the URL. A common shape: every call goes to
`/accounts/<account id>/...`, and that id differs per install. Reference any **non-secret**
connection variable by name and it is substituted at compile time:

```yaml
base_url: https://api.acme.com/accounts/{ACME_ACCOUNT_ID}/v2
```
...
The **host is still literal**. Per-install *hosts* — self-hosted GitLab, a `<site>.example.com`
tenant, any on-prem server — remain unsupported on purpose, because the destination allow-list is
what stops the host making authenticated calls to an origin chosen at runtime. Supporting them is a
real design decision, not a missing feature.

### `mutating` — approval gating

Any method other than `GET` is treated as mutating, and mutating Tools are approval-gated. Override
it when a provider models a read as a `POST` — a `search` or `query` operation often does, and
...
- **Request bodies nest under `body`.** A body property and a path parameter can share a name
  (a `page_id` that is both a path segment and a body field); merging them would silently drop one.
...
Pick the *fewest* steps that work. Google Workspace uses a `fields` step then `oauth2`; GitHub needs
`app_manifest` then `install` because creating a GitHub App grants access to nothing.
...
```yaml
- kind: webhook
  title: Point Acme at this deployment
  url: https://api.acme.com/bot{ACME_BOT_TOKEN}/setWebhook
  secret_env: ACME_WEBHOOK_SECRET
  body:
    url: "{webhook_url}"
    secret_token: "{ACME_WEBHOOK_SECRET}"
...
ingress:
  ...
  webhook:
    security:
      type: shared_secret
      header: X-Acme-Bot-Api-Secret-Token
      secret_env: ACME_WEBHOOK_SECRET
    dedup_key: update_id
  context_env: [ACME_BOT_USERNAME]
  chat:
...
|---|---|---|
| `hmac_sha256` | Signs the raw body with a shared secret (GitHub, Slack) | `header`, `secret_env`, optional `prefix` |
| `shared_secret` | Echoes back a secret you registered (a bot API) | `header`, `secret_env` |
...
`ctx.env` is exactly what `context_env` names, and nothing else. A bot webhook update often never
says which bot it was sent to, so a classifier cannot recognise a mention of itself without being
told its own `@username`. **Naming a var the auth flow stores as a secret is rejected at load** — a classifier is
...
conversation. Get its axes right or distinct conversations merge: a forum-style provider needs the
topic id in the key, because without it every topic collapses into one conversation.
...
email simply leave senders unlinked until they bind an account, which is a documented path with an
offered link rather than a failure.
...
4. Write `setup-guide.md` — especially any step the provider requires that a token alone does not
   cover. Google Workspace's per-file sharing is a good example: a valid token with nothing shared
...
6. Write a test that compiles the real shipped files, in the shape of
   `apps/api/src/tools/declarative/bundled-manifests.test.ts`. A fixture would keep passing after
   the real manifest broke.
...
7. Write `ingress.ts`, and vendor a byte-identical copy as
   ...
   through the **real** sandbox — see `apps/api/src/ingress/slack-parity.fixture.test.ts`.
