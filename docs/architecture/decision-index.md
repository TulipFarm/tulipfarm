...
| ADR-028 | The Curator is one hourly job that rewrites a user's Memory Document and nothing else; deterministic maintenance stays deterministic and separate | The Proposal pipeline never delivered a model-derived effect to a user, while Memory was maintained by nothing; a fast model over a per-user watermark costs nothing on an idle hour | `apps/worker/src/memory-curation`, `packages/built-in-agents` |
...
| Curation as a durable Run per user | A Memory rewrite has no Tools, no approvals and no participant; a Run buys recovery machinery for a job whose whole recovery is "leave the watermark and try next hour" |
