...
| `agent-runtime` | 15 | composed — the turn engine the Worker executes: context assembly, the bounded loop, and all three guardrail stages (PR 3). Routine `agent` States run the same loop against the Agent and ModelProfile the Run's pinned bundle names (`apps/worker/src/routine/agent-port.ts`, PR 4), with no Tools exposed, the deployment's default guardrail policy (no Soul publishes one into the bundle yet), and the Agent's `instructions.md` body absent because it is a Soul companion file the bundle carries only the hash of |
