...
| LB-14 | surface | `apps/api/src/surface/surface-store.ts` | Tulip Surface Protocol surfaces are process/local store rows rather than immutable authorized Artifacts with signed, expiring action descriptors; client actions are not re-authorized against a signed intent | I-14 | AW-081 | AW-084 | AW-096 | medium |
