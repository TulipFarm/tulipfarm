...
| 3 | `expect` the "Recent operational activity" table renders the real activity/audit log (`ActivityService.list`), with a `Filter recent activity` search input (labeled via `sr-only` text) and a "View all activities" link to `/settings/activities` | Present |
