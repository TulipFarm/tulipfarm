...
reach out to a real third party (Slack, GitHub, Google) or leak a real secret
...
- **Never send a real outbound message.** No Slack post, no GitHub comment, even if a connected
  integration would make one reachable.
...
2. **OAuth-start is also unregistered.** `startOAuth()` (`POST
   /api/v1/integrations/:name/oauth/start`) has no server route. Separately, of the three bundled
   manifests (`integrations/{github,google,slack}/manifest.yml`) only `google` declares an `oauth2:`
   block, so for Slack and GitHub `_app.integrations.$name.tsx`'s `useGuidedWizard =
   Boolean(integration.setupGuide) && !oauthConfig` condition keeps the OAuth-vs-manual split UI
   branch out of reach — only google can drive it. Note what is seen; do not treat this as a bug to
   chase down.
...
disconnected integration (github, google) if Slack is already
...
| 3 | note | Outbound-render preview (Slack Block Kit / GitHub comment) has no in-app surface — `surface-slack`/`surface-github` exist as packages but are not imported into any web route. Record as N/A, not a missing control |
...
- **Two states are undeterminable from source and must be recorded as observed, not predicted**: the
  empty-list state on `/integrations` (likely unreachable since three bundled manifests always
