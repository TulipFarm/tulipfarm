...
| 5 | If the skill declares no `commands` in its `SKILL.md` frontmatter, `expect` no "Executable Tools" section renders at all (not an empty one) | Confirmed absent, not broken |
...
| 3 | `expect` any `[tool: <name>]` rows in the transcript name tools plausibly related to what S4's "tool reach" disclosed, or one of the always-exposed structural tools (`skill`, `complete_task`, `delegate_to_agent`, `present`, `request_input`, `update_presentation`) or any mutating tool the agent holds | Recorded, not a hard pass/fail per the caveat above |
...
| 5 | **Heading structure**: `expect` exactly one `h1` per page and no skipped level. `page-shell.tsx` renders the `h1` from `title` on all three routes ("Skills", "Marketplace", the skill's name), so the `<h2>`s below it (marketplace's category groups, the detail page's "Executable Tools") no longer skip a level. Expect this to pass; zero `h1`, or a second one in the content, is a shell regression to file once, not per-route | Pass |
