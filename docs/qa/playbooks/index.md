...
| --- | --- | --- | --- | --- |
| `20260918-2154-integrations` | 2026-09-18–19 | MCP catalog, five setup entry paths, one-action completion, native channel entry, invalid links, desktop/mobile | None | Removed mandatory Tool checklist; fixed audit failure, retry/legacy-policy handling and account-state actions. 221 web tests, 135 backend tests and 20 isolated browser cases pass. Live recheck read-only; no real credential/provider operations. |
...
Integrations (08) now has a local MCP UI-only run; live provider authorization, delivery and sync
remain unverified. Channel Linking (14) was deferred in staging; the integration run checks invalid
links only, not successful binding. Admin & RBAC (11) non-admin staging scenarios remain blocked
by the invite-link bug (#433).
