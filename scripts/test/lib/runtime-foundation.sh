...
runtime_prepare_legacy_fixture() {
  ...
  [ "$(compose exec -T postgres psql -U tulipfarm -d tulipfarm -At \
    -c "SELECT version FROM schema_version")" = 140 ] \
...
BEGIN;
DROP TABLE mcp_setup_operations;
DROP TABLE file_knowledge_requests;
...
DELETE FROM schema_migrations WHERE version IN (134, 135, 136, 137, 138, 139, 140);
