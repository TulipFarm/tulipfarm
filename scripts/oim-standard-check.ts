...
try {
  ...
  const generatedSchemas = join(checkDirectory, "schemas");
  await generateOimStandardRuntime(ROOT, generatedRuntime);
  await generateOimStandardSchemas(ROOT, generatedSchemas);
  if (
  ...
  for (const [relativePath] of oimStandardSchemas()) {
    if (
      (await readFile(join(generatedSchemas, relativePath), "utf8")) !==
      (await readFile(join(STANDARD, "schemas", relativePath), "utf8"))
    ) {
      mismatches.push(`schemas/${relativePath} is stale`);
    }
  }
} finally {
  await rm(checkDirectory, { recursive: true, force: true });
