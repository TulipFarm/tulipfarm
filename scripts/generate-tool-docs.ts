...
const ELSEWHERE: Readonly<Record<string, string>> = {
  surface: "/using-tulipfarm/surfaces",
};
...
[what agents can do](/using-tulipfarm/what-agents-can-do) for how that reads from the outside,
and [roles and permissions](/reference/roles-and-permissions) for how the checking works.
...
not a fixed catalog. See [integrations](/reference/bundled-integrations) for how they differ
from native Slack and GitHub channels. Tools that draw a reply on screen are explained in
[surfaces](/using-tulipfarm/surfaces). General-purpose network tools are omitted from this
