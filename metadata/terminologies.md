...
| A connected third party | **Integration** | `Integration` | `/integrations`, `/api/v1/integrations` | "Integrations" | MCP supplies third-party Agent Tools; native Slack/GitHub channels supply events and replies. `connection`/`connector` → retired |
| The non-secret, reviewed configuration of an MCP Integration | **Integration definition** | `McpIntegrationDefinition` | `/api/v1/integrations/:slug` | "Integration settings" | `integrations/<slug>/mcp.yaml`; transport, authentication requirements, enablement and reviewed capabilities. Technical hosting fields belong under Advanced settings. Credentials and account grants stay outside the Soul |
| An identity and its credentials for one MCP Integration | **Integration account** | `McpAccount` | `/api/v1/integrations/:key/accounts` | "Accounts" | Personal or shared; binds one principal or an explicitly granted shared identity. Not a TulipFarm user account. A default never replaces an existing Chat selection |
| Fixed authentication and setup metadata for a native channel | **Integration manifest** | `IntegrationManifest` | — | — | Bundled Slack/GitHub metadata only; not an extensible provider framework or third-party business Tool catalog |
| Requests sent to an external destination | **Egress** | `EgressHttp` | — | — | Destination-guarded network access, including MCP. The retired OpenAPI/TypeScript egress-operation framework is not supported |
| Provider events accepted by TulipFarm | **Ingress** | `NativeChannelInboxRecord` | — | — | Fixed Slack/GitHub verification and durable event admission; no universal webhook interpreter |
...
| Reusable asset templates with an installation dependency plan | **Pack** | `PackDefinition` | `/packs`, `/packs/import`, `/api/v1/packs` | "Packs", "Import pack" | Adapted to existing Resource types, Skills, Agents, Surfaces, and Routines before confirmation; not a Skill, installed host package, or grant of authority |
...
| The hourly background job that rewrites a user's Memory Document | **Curator** | `Curator` | — | not surfaced | runs on the `memory-curation` queue at `0 * * * *`; reads only what the user typed since that user's watermark and calls a fast model only when there are new Turns; ⛔ "the loop", "reconciler" (that names the deterministic Task checks on `maintenance-sweep`) |
| How far a user's Memory has been curated | **Curation Watermark** | `memory_curation_watermark` | — | — | one row per user, moves forward only; a user with no row curates from the epoch, which is how backfill happens with no special case; ⛔ "cursor", "curated flag" |
...
| A system-created unit of work asking a human to do something | **Task** | `Task` | `/api/v1/tasks` | "Task" | created by the deterministic checks or by an Agent via `task.create`; never user-created — a user-facing todo/ticket product is a Resource Type, not a Task; ⛔ "quest" (retired) |
...
| Agent-to-channel presentation standard | **Tulip Surface Protocol** | `TSP`, `SurfaceArtifact`, `SurfaceRenderer`, `SurfaceInteraction` | `/api/v1/surfaces`, `/dev/surfaces`, `surface-components/` | "Tulip Surface Protocol", "presentation" | Channel-neutral semantic components; never persisted provider payloads. Executable UI exists only as a Code view (below), never as a provider payload or an inline template |
| Agent-authored JSX carried by a Surface component and run in a sandboxed frame | **Code view** | `SurfaceCodeView`, `soulSurfaceCodeViewFor` | `code/` companions in `surface-components/` | — | Web only; compiled at authoring time; executes in an opaque-origin `sandbox="allow-scripts"` frame under `connect-src 'none'` (ADR-031). ⛔ "custom component", "widget", "plugin" |
...
| memory extraction | Curator | retired — consolidation runs hourly off the turn path |
...
| quest / Proposal / Proposal Seed / Curator Work / Work Reason / Curator Effect | — | retired — the Proposal pipeline is deleted; the Curator now writes Memory only |
