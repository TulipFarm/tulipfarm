...
| A Provider Connection backed by a personal CLI subscription token instead of an API key | **Subscription Provider** | `CliLanguageModel` (`packages/llm/src/cli/`) | — | "(subscription)" suffix on the provider label | e.g. `claude-code`; runs a coding-agent CLI (`@anthropic-ai/claude-agent-sdk`) as the model in a jailed subprocess, one call per AgentLoop iteration, tool calls captured and never executed by the CLI. See `docs/plans/cli-agent-providers.md`. ⛔ "harness" — already means the default chat surface (see Agent row above) |
